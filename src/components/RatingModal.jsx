...
// Step flow: 'rate' → 'comment' (after rating selected + "Add comment") or save immediately on "Skip"
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-40 flex items-end justify-center pointer-events-none">
        <div
          className="pointer-events-auto w-full max-w-[430px] rounded-t-[32px] shadow-2xl pb-8"
        ...
        >
          ...
          <div className="px-6 space-y-5">
            ...
            <div className="flex items-center gap-4">
              {item.media_poster_url && (
                <img src={item.media_poster_url} alt={item.media_title}
                  className="w-14 h-20 object-cover rounded-2xl shadow-lg shrink-0" />
              )}
              <div className="min-w-0">
                <p className="font-extrabold text-white text-lg leading-tight">{item.media_title}</p>
                <p className="text-white/50 text-sm capitalize mt-0.5">{item.media_type}{item.year ? ` · ${item.year}` : ''}</p>
                {existingEntry && (
                  <p className="text-white/40 text-xs mt-1">Already in your log — updating rating</p>
                )}
              </div>
            ...
            {/* Step: choose action after rating */}
            {step === 'rate' && rating && (
